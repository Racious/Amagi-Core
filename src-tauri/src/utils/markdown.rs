...
pub fn build_vault_pointer_block(vault_folder: &str, for_codex: bool) -> String {
    ...
    [
        ...
        format!("- 本專案記憶見 vault `{}/agent/memory/`：先讀 `MEMORY.md` 索引，需要時再讀個別記憶檔。", vault_folder),
...
/// 專案層 AGENTS.md：純薄指標（記憶已移 vault `agent/memory/`，不再 inline）。
pub fn build_agents_md(vault_folder: &str) -> String {
    [
        "# AGENTS.md".to_string(),
        String::new(),
...
pub fn build_claude_md(vault_folder: Option<&str>) -> String {
    let mut lines = vec![
        "# CLAUDE.md".to_string(),
        String::new(),
        "> Auto-generated by AMAGI Core. Do not edit manually.".to_string(),
...
mod tests {
    ...
    fn test_vault_pointer_in_generated_files() {
        // AGENTS.md：含專案指針、保留標頭、引用 codex 錨點、記憶指標
        ...
        // 記憶改指標：指向 agent/memory，不再 inline
        assert!(agents.contains("projects/foo/agent/memory/"));
