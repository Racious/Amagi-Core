...
fn e2e_learn_review_sync_memory() {
    ...
    // Phase 3a：AGENTS.md / CLAUDE.md 改純薄指標（記憶移 vault，不再 inline）
    let agents = sb.read("AGENTS.md");
    assert!(agents.starts_with("# AGENTS.md"));
    assert!(agents.contains("Auto-generated by AMAGI Core"));
    assert!(agents.contains("agent/memory/"), "AGENTS.md 應含記憶指標");
    assert!(!agents.contains("Project Rules"), "記憶不應再 inline 於 AGENTS.md");
