...
新增記憶投遞與刪除流程，修正審查文件路由，並調整 Agent 指引與任務回顧模板。

### 功能
- **記憶投遞通道**：支援 `.amagi/pending/` 的 `memory-*.md` 草稿，經審核後存入專案、共用或全域記憶；審核介面可辨識記憶類型並調整範圍。
- **記憶刪除**：提供刪除確認與安全檢查，刪除後重讀資料並更新相關索引；顯示可復原性與更新失敗提示。

### 修正
- **投遞處理狀態**：安全過濾略過的項目可見化；歸檔失敗不再靜默忽略，避免重複投遞難以判斷。
- **審查文件路由**：`review-brief`、`review-resolution` 與審查報告統一落入專案 `reports/`。
- **背景讀取指引**：保留 handoff 優先，日誌、knowledge 與 reports 改為依任務需要讀取。
- **回顧模板**：只有重複需求、方法穩定且現有指引不足時才建技能草稿；通用清單與任務結果分開，記憶投遞引用現行指引。

### 升級後操作
- 先同步最新 vault，再以新版 Core 執行「同步全域 doctrine」及需要更新的專案同步。
- 自訂技能正本更新後仍須重新分發；已勾選的目標不代表內容已更新。
- 既有 `.amagi/after-task-review.md` 與投遞指引不會自動覆寫，需保留原內容後另行更新；新專案初始化會使用新版模板。
- 安裝更新不等於各專案指引與技能已部署；各台機器需分別核對。
